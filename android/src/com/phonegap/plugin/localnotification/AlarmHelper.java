...
public class AlarmHelper {
  ...
  public boolean cancelAlarm(String notificationId) {
     ...
     * add. Making sure the notification id in the action is the same. Now we
     * can search for such an intent using the 'getService' method and cancel
     * it.
     */
    final Intent intent = new Intent(this.ctx, AlarmReceiver.class);
    intent.setAction("" + notificationId);

    final PendingIntent pi = PendingIntent.getBroadcast(this.ctx, 0, intent,
        PendingIntent.FLAG_CANCEL_CURRENT);
    final AlarmManager am = getAlarmManager();
    
    final Object systemService = ctx.getSystemService(Context.NOTIFICATION_SERVICE);
    final NotificationManager notificationMgr = (NotificationManager) systemService;
    notificationMgr.cancel(Integer.parseInt(notificationId));
  ...
  public boolean cancelAll(SharedPreferences alarmSettings) {
    ...
    for (String alarmId : alarmIds) {
      Log.d(LocalNotification.PLUGIN_NAME, "Canceling notification with id: "
          + alarmId);
