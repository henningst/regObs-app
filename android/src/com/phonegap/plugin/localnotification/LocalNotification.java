...
public class LocalNotification extends Plugin {
  ...
  public PluginResult execute(String action, JSONArray optionsArr,
    ...
    if (action.equalsIgnoreCase("add")) {
      ...
      return this.add(daily, title, subTitle, ticker, alarmId,
          alarmOptions.getCal());
   ...
   * @param alarmTitle
   *          The title of the alarm as shown in the Android notification panel
   * @param alarmSubTitle
  ...
  public PluginResult add(boolean repeatDaily, String alarmTitle,
      String alarmSubTitle, String alarmTicker, String alarmId, Calendar cal) {
    final long triggerTime = cal.getTimeInMillis();
    final String recurring = repeatDaily ? "daily" : "onetime";

    Log.d(PLUGIN_NAME, "Adding " + recurring + " notification: '" + alarmTitle
        + alarmSubTitle + "' with id: " + alarmId + " at timestamp: "
        + triggerTime);
  ...
  public PluginResult cancelAllNotifications() {
    ...
    /*
     * Android can only unregister a specific alarm. There is no such thing as
     * cancelAll. Therefore we rely on the Shared Preferences which holds all
     * our alarms to loop through these alarms and unregister them one by one.
     */
